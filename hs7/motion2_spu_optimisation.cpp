 ...
 * #1.3
 * Task #2:
 * Although replicating the k-NN task and implementing a 5-stage 
 * pipeline could potentially increase throughput, this approach is not 
 * ideal due to the following reasons:
 *
 * 1. Data Dependency : The k-NN task depends on data from both the current and previous frames.
 *    Replicating this task would create conflicts, as multiple instances would need to access shared data.
 *
 * 2.  Increased Latency : Adding more stages to the pipeline increases processing latency for each frame.
 *  While throughput might improve, the latency would also rise.
 *
 * 3. Resource Overhead : Replicating k-NN would require additional memory and bandwidth, particularly 
 *    for large datasets, which may actually degrade performance.
 *
 * Therefore, replicating the k-NN task and expanding the pipeline to 5 stages would likely reduce real-time 
 * performance by increasing latency and resource usage.
 */
...
int main(int argc, char** argv) {
	...
	// -- Initialization t -- //
	// ------------------------ //
    Sigma_delta sigma_delta_mod(sd_data, i0, i1, j0, j1, p_sd_n);
    Morpho morpho_mod(morpho_data, i0, i1, j0, j1);
    CCL ccl_mod(ccl_data, def_p_cca_roi_max1);
    Features_CCA features_mod(i0, i1, j0, j1, p_cca_roi_max1);
    Features_filter features_filter_mod(i0, i1, j0, j1, p_cca_roi_max1,
    ...
    sigma_delta_mod.sigma_delta_init((const uint8_t**)video["generate::out_img_gray8"].get_2d_dataptr<uint8_t>());
    ...
    sigma_delta_mod["compute::in_img"] = delayer["produce::out"];
    // step 2: mathematical morphology
    morpho_mod["compute::in_img"] = sigma_delta_mod["compute::out_img"];

    // ----------------- //
    // -- Stage 2 ----- //
    // ----------------- //
    // step 3: connected components labeling (CCL)
    ccl_mod["apply::in_img"] = morpho_mod["compute::out_img"];
    // step 4: connected components analysis (CCA): from image of labels to "regions of interest" (RoIs)
    features_mod["extract::in_labels"] = ccl_mod["apply::out_labels"];
    features_mod["extract::in_n_RoIs"] = ccl_mod["apply::out_n_RoIs"];            
    // step 5: surface filtering (rm too small and too big RoIs)
	if (p_forward) {
        features_filter_mod["filterf::fwd_labels"] = ccl_mod["apply::out_labels"];
        features_filter_mod["filterf::in_n_RoIs"] = ccl_mod["apply::out_n_RoIs"];
        features_filter_mod["filterf::in_RoIs"] = features_mod["extract::out_RoIs"];
    } else {
        features_filter_mod["filter::in_labels"] = ccl_mod["apply::out_labels"];
        features_filter_mod["filter::in_n_RoIs"] = ccl_mod["apply::out_n_RoIs"];
        features_filter_mod["filter::in_RoIs"] = features_mod["extract::out_RoIs"];
    }
    
    // Use Delayer to memorize the filtering value for the next frame
    delayer["memorize::in"] = features_filter_mod[p_forward ? "filterf::out_labels" : "filter::out_labels"];
    ...
    if (p_forward) {
        knn_mod["matchf::in_n_RoIs0"] = features_filter_mod["filterf::out_n_RoIs"];
        knn_mod["matchf::in_RoIs0"] = delayer["produce::out"];
        knn_mod["matchf::in_n_RoIs1"] = features_filter_mod["filterf::out_n_RoIs"];
        knn_mod["matchf::fwd_RoIs1"] = features_filter_mod["filterf::out_RoIs"];
    } else {
        knn_mod["match::in_n_RoIs0"] = features_filter_mod["filter::out_n_RoIs"];
        knn_mod["match::in_RoIs0"] = delayer["produce::out"];
        knn_mod["match::in_n_RoIs1"] = features_filter_mod["filter::out_n_RoIs"];
        knn_mod["match::in_RoIs1"] = features_filter_mod["filter::out_RoIs"];
    }
    // step 7: temporal tracking
	if (p_forward) {
		tracking_mod["perform::in_n_RoIs"] = features_filter_mod["filterf::out_n_RoIs"];
		tracking_mod["perform::in_RoIs"] = knn_mod["matchf::fwd_RoIs1"];
	}else{
		tracking_mod["perform::in_n_RoIs"] = features_filter_mod["filter::out_n_RoIs"];
    ...
    if (p_ccl_fra_path) {
		if (p_forward) {
        	(*log_fra)["write::in_labels"] = features_filter_mod["filterf::out_labels"];
		}else{
			(*log_fra)["write::in_labels"] = features_filter_mod["filter::out_labels"];
		}
    ...
    if (p_log_path) {
		if(p_forward){
			log_RoIs["write::in_RoIs0"] = features_filter_mod["filterf::out_RoIs"];
			log_RoIs["write::in_n_RoIs0"] = features_filter_mod["filterf::out_n_RoIs"];
		}else{
			log_RoIs["write::in_RoIs0"] = features_filter_mod["filter::out_RoIs"];
			log_RoIs["write::in_n_RoIs0"] = features_filter_mod["filter::out_n_RoIs"];
		}
...
#endif
		if(p_forward){
			log_kNN["write::in_RoIs0"] = features_filter_mod["filterf::out_RoIs"];
			log_kNN["write::in_n_RoIs0"] = features_filter_mod["filterf::out_n_RoIs"];
		}else{
			log_kNN["write::in_RoIs0"] = features_filter_mod["filter::out_RoIs"];
			log_kNN["write::in_n_RoIs0"] = features_filter_mod["filter::out_n_RoIs"];
		}
    ...
    if (visu) {
		...
		if(p_forward){
			...
			(*visu)["display::in_n_RoIs"] = features_filter_mod["filterf::out_n_RoIs"];
		}else{
			(*visu)["display::in_RoIs"] = knn_mod["match::out_RoIs1"];
			(*visu)["display::in_n_RoIs"] = features_filter_mod["filter::out_n_RoIs"];
		}
        (*visu)("display").exec();
    }
    ...
    std::vector<std::tuple<
        ...
        std::vector<spu::runtime::Task*>>> pip_stages = {
            std::make_tuple<std::vector<spu::runtime::Task*>, 
				std::vector<spu::runtime::Task*>,
				std::vector<spu::runtime::Task*>>({ &video("generate") },
                    { &sigma_delta_mod("compute") },
                    {}
            ),
    ...
    std::vector<spu::runtime::Task*> seq_first_tasks = { &video("generate") };

...
int main(int argc, char** argv) {
    ...
    // save frames (CCs)
    if (p_ccl_fra_path) {
        (*log_fra)["write::in_labels"] = ccl_mod1["apply::out_labels"];
    ...
    std::vector<spu::runtime::Task*> first_tasks = {&delayer("produce"), &video("generate")};
                                    //last_tasks = {&tracking_mod("perform")};
    /*if(p_ccl_fra_path)
        last_tasks.push_back(&((*log_fra)("write")));
    if(p_log_path)
        last_tasks.push_back(&log_RoIs("write"));
    if(visu)
        last_tasks.push_back(&((*visu)("display")));
    */
    spu::runtime::Sequence sequence(first_tasks);
    sequence.exec([&video]() {return video.is_done();});

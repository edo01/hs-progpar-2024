...
Tracking:: Tracking(tracking_data_t* tracking_data, int p_cca_roi_max2, size_t frame, size_t r_extrapol, 
            size_t fra_obj_min, uint8_t save_RoIs_id, uint8_t extrapol_order_max, 
            float min_extrapol_ratio_S)
    : spu::module::Stateful(), tracking_data(tracking_data), p_cca_roi_max2(p_cca_roi_max2), frame(frame), r_extrapol(r_extrapol), 
    ...
    create_codelet(t, 
        ...
        (Module &m, spu::runtime::Task &tsk, size_t frame) -> int 
        {
            ...
            tracking_perform(tracking.tracking_data, RoIs_in, 
                        *n_RoIs_in, tracking.frame, tracking.r_extrapol, 
